...
urlpatterns = [
 ...
 path('create-event/', EventViewSet.as_view({'post': 'create'}), name='create-event'),
    ...
    path('user_event_in_progress/<str:user>/', EventViewSet.as_view({'get': 'get_events_in_progress_for_user'}), name='event_user_in_progress'),
    path('search-event_by-name/<str:name>/', EventViewSet.as_view({'get': 'search_event_by_name'}), name='search-event-by-name'),
